...
if [ $a == '198-6' ]; then
DATA_DIR=/mnt/lustre/share_data/zhujinguo/open_source_dataset/bert_pretrain_data/bookswiki/data-bin/bookswiki-combine
elif [ $a == '198-8' ]; then
DATA_DIR=/mnt/lustrenew/share_data/zhujinguo/open_source_dataset/bert_pretrain_data/bookswiki/data-bin/bookswiki-combine
elif [ "${SRUN}" == "local" ]; then 
...
if [ "${SRUN}" == "local" ]; then
  ...
  train.py --fp16 $DATA_DIR \
    ...
    --max-update $TOTAL_UPDATES --log-format simple --log-interval 10 --save-dir $WORK_DIR/checkpoints \
